...
Typically, the performance of a classification model can be measured by accuracy, which is defined as the number of correct predictions from all the predictions made. But in this case it is not the best metric due to the class imbalance (~25% class 'left' vs ~75% class 'stay'). Instead, we can use the a combination of precision and recall as an unambiguous way to show the prediction results of the classifiers. Precision is the number of True Positives (TP) divided by the number of True Positives (TP) and False Positives (FP). A low precision indicate a large number of False Positives.    
...
The dataset contains 14,999 rows and 10 columns. The features include:   
...
*Note that we have a combination of numerical features and categorical features.* They need to be treated differently.
...
It is a binary classification prlblem and there are a number of algorithms available in the `scikit-learn` library. To find the best model for the problem, the following algirithms are used to train the dataset:   
...
For each model, cross validation outputs 10 test F2 scores. The models can be compared by creating boxplots for the scores, as shown below:    
![Model comparison](./plots/modelcomparison.png)  

Based on the comparison, Decision Tree classifier and Random Forest have the highest F2 scores, and are much better than the benchmark score. Generally, Random Forest is less likely to overfit the data, so it is chosen as the best model.  
...
Based on the model comparison plot above, Random Forest Classifier is chosen since it gives the best F2 score. After fine tuning the hyperparameters, the final Random Forest classifier (with 50 decision trees) gives an even higher F2 score. 

The discrepency between training score and test scores is also evaluated by first randomly splitting the original dataset to 20% test set and 80% training set and then training the model with different training size. The learning curves for training and testing are compared below. The learning curves converge as the number of training points increases. With 100% of the training data (80% of the original dataset) used for training the model, the training score and the test score are close, indicating that the model generalizes well to unseen data.  
![Learning Curve](./plots/learningcurve.png)  
With the Random Forest model trained on all the training data, the confusion matrix of the prediction on the test set is obtained to be  
...
With a classification model like the Random Forest Classifier built above, employers is able to predict if a current employee is leaving such that immediate internvention can take place. Another key information for an effective retention program is the knowledge of the most important factors on employee turnover. Luckily, with Random Forest Classifier, we can easily obtain the feature importance.  
![Feature importance](./plots/featureimportance.png)  
The top 5 features from the optimized Random Forest model trained on the training set is shown in the bar plot above. Thses 5 features consists over **95%** of the feature importance. `'satisfaction_level'` is the most important feature which is consistant with its high correlation coefficient with the target variable `'left'`. The other features highly relevant to employee turnover are `'average_monthly_hours'`, `'number_project'`, `'time_spend_company'`, and `'last_evaluation'`.   
...
Besides, the current model predicts the class labels: left or stay. An alternative is to output the class probability instead of the class label. This affords the management and human resource teams more flexibility as they can specifies a customized probability threshold when deciding on intervention.     
